...
def get_mfcc_features(wav, frq):
    result = calcMFCC(wav, sample_rate=frq)
    features = np.hstack([np.mean(result, axis=0),np.var(result, axis=0),sp.stats.skew(result, axis=0)])
    return features

if __name__ == '__main__':
    from helper_code import load_wav_file

    wav_file = "E:\\1_dataset\\Pysionet\\CINC2022\\dataset\\training_data\\49562_AV.wav"
    wav, frq = load_wav_file(wav_file)
    result = get_mfcc_features(wav, frq)
    # 2539,26
    print(result)